window.UI = (()=>{
  ...
  function addToolCard(id,name,status){
    ...
    hdr.innerHTML='<span class="tool-name">&xutri; '+esc(name)+'</span><span class="tool-status running">running</span>';
  ...
  function updateToolCard(id,output,status){
    ...
    st.textContent=status||'complete';
    st.className='tool-status '+(status==='error'?'error':'complete');
