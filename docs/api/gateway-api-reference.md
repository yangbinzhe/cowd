...
AI Harness 的运行状态、事件、控制平面、turn 提交和 session lease。
...
| `GET` | `/api/sessions/:id/execution-policy` | Session 生命周期 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `get_session_execution_policy` | `session_routes.rs` | P1 |
| `PUT` | `/api/sessions/:id/execution-policy` | Session 生命周期 全量更新接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `put_session_execution_policy` | `session_routes.rs` | P1 |
...
| `DELETE` | `/api/sessions/:id/mission-focus` | Session 生命周期 删除接口 | id | - | 通常无 body 或仅 path/query | `clear_mission_focus_handler` | `session_routes.rs` | P1 |
| `GET` | `/api/sessions/:id/mission-focus` | Session 生命周期 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `get_mission_focus_handler` | `session_routes.rs` | P1 |
| `PUT` | `/api/sessions/:id/mission-focus` | Session 生命周期 全量更新接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `set_mission_focus_handler` | `session_routes.rs` | P1 |
...
| `DELETE` | `/api/sessions/:id/task-focus` | Session 生命周期 删除接口 | id | - | 通常无 body 或仅 path/query | `clear_task_focus_handler` | `session_routes.rs` | P1 |
| `GET` | `/api/sessions/:id/task-focus` | Session 生命周期 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `get_task_focus_handler` | `session_routes.rs` | P1 |
| `PUT` | `/api/sessions/:id/task-focus` | Session 生命周期 全量更新接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `set_task_focus_handler` | `session_routes.rs` | P1 |
...
| `GET` | `/api/sessions/execution-policy-defaults` | Session 生命周期 查询接口 | - | 可选 Query 视具体 handler 而定 | - | `get_execution_policy_defaults` | `session_routes.rs` | P1 |
| `PUT` | `/api/sessions/execution-policy-defaults` | Session 生命周期 全量更新接口 | - | - | JSON 或 Multipart，详见对应 Request struct | `put_execution_policy_defaults` | `session_routes.rs` | P1 |
...
| `GET` | `/api/tasks/:id` | Task 阶段化执行 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `task_detail_handler` | `task_routes.rs` | P2 |
...
| `POST` | `/api/tasks/:id/focus` | Task 阶段化执行 创建/动作接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `focus_task_handler` | `task_routes.rs` | P2 |
| `POST` | `/api/tasks/:id/mission/commit` | Task 阶段化执行 创建/动作接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `commit_task_mission_handler` | `task_routes.rs` | P1 |
| `POST` | `/api/tasks/:id/mission/preview` | Task 阶段化执行 创建/动作接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `preview_task_mission_handler` | `task_routes.rs` | P1 |
...
| `GET` | `/api/tasks/:id/turns` | Task 阶段化执行 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `task_turns_handler` | `task_routes.rs` | P2 |
| `POST` | `/api/tasks/mission/commit` | Task 阶段化执行 创建/动作接口 | - | - | JSON 或 Multipart，详见对应 Request struct | `commit_tasks_mission_handler` | `task_routes.rs` | P1 |
| `GET` | `/api/tasks/mission/organization` | Task 阶段化执行 查询接口 | - | 可选 Query 视具体 handler 而定 | - | `mission_organization_handler` | `task_routes.rs` | P1 |
| `POST` | `/api/tasks/mission/preview` | Task 阶段化执行 创建/动作接口 | - | - | JSON 或 Multipart，详见对应 Request struct | `preview_tasks_mission_handler` | `task_routes.rs` | P1 |
...
| `GET` | `/api/skills/:id/active-pointer` | 技能系统 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `skill_active_pointer_handler` | `skill_routes.rs` | P2 |
...
| `GET` | `/api/skills/revision-reviews/:id` | 技能系统 查询接口 | id | 可选 Query 视具体 handler 而定 | - | `skill_revision_review_handler` | `skill_routes.rs` | P2 |
| `POST` | `/api/skills/revision-reviews/:id/decision` | 技能系统 创建/动作接口 | id | - | JSON 或 Multipart，详见对应 Request struct | `skill_revision_review_decision_handler` | `skill_routes.rs` | P2 |
...
| `POST` | `/api/approval/prune` | Approval 审批 创建/动作接口 | - | - | JSON 或 Multipart，详见对应 Request struct | `approval_prune_handler` | `approval_routes.rs` | P1 |
...
| 方法 | 路径 | 用途 | Path 参数 | Query | Body | Handler | Source | 级别 |
|---|---|---|---|---|---|---|---|---|
| `GET` | `/api/apps` | 签名 APP 查询接口 | - | 可选 Query 视具体 handler 而定 | - | `list_apps` | `app_routes.rs` | P2 |
| `GET` | `/api/apps/:app_id` | 签名 APP 查询接口 | app_id | 可选 Query 视具体 handler 而定 | - | `get_app` | `app_routes.rs` | P2 |
| `GET` | `/api/apps/:app_id/logs` | 签名 APP 查询接口 | app_id | 可选 Query 视具体 handler 而定 | - | `get_app_logs` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/operations/:operation_id/invoke` | 签名 APP 创建/动作接口 | app_id, operation_id | - | JSON 或 Multipart，详见对应 Request struct | `invoke_operation` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/operations/:operation_id/stream` | 签名 APP 创建/动作接口 | app_id, operation_id | - | JSON 或 Multipart，详见对应 Request struct | `stream_operation` | `app_routes.rs` | P2 |
| `GET` | `/api/apps/:app_id/receipts/:receipt_id` | 签名 APP 查询接口 | app_id, receipt_id | 可选 Query 视具体 handler 而定 | - | `get_receipt` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/restart` | 签名 APP 创建/动作接口 | app_id | - | JSON 或 Multipart，详见对应 Request struct | `restart_app` | `app_routes.rs` | P2 |
| `DELETE` | `/api/apps/:app_id/subscriptions/:subscription_id` | 签名 APP 删除接口 | app_id, subscription_id | - | 通常无 body 或仅 path/query | `cancel_subscription` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/subscriptions/:subscription_id/ack` | 签名 APP 创建/动作接口 | app_id, subscription_id | - | JSON 或 Multipart，详见对应 Request struct | `ack_subscription` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/tui/views/:view_id/actions` | 签名 APP 创建/动作接口 | app_id, view_id | - | JSON 或 Multipart，详见对应 Request struct | `tui_action` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/tui/views/:view_id/open` | 签名 APP 创建/动作接口 | app_id, view_id | - | JSON 或 Multipart，详见对应 Request struct | `tui_open` | `app_routes.rs` | P2 |
| `POST` | `/api/apps/:app_id/tui/views/:view_id/stream` | 签名 APP 创建/动作接口 | app_id, view_id | - | JSON 或 Multipart，详见对应 Request struct | `tui_stream` | `app_routes.rs` | P2 |
| `GET` | `/apps/:app_id` | 签名 APP 查询接口 | app_id | 可选 Query 视具体 handler 而定 | - | `serve_index` | `app_routes.rs` | P2 |
| `GET` | `/apps/:app_id/*path` | 签名 APP 查询接口 | app_id | 可选 Query 视具体 handler 而定 | - | `serve_asset` | `app_routes.rs` | P2 |
