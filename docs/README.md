...
- 存储：PostgreSQL 唯一；无 SQLite 驱动/后端/回退，`storage.backend` 非法值 fail-closed。
...
| 存储 | PostgreSQL 唯一（fail-closed，无 SQLite 回退） | [architecture](architecture/README.md) |
...
| APP 平台 | 签名 Bundle 启动发现、隔离 Worker、通用 invoke/stream/TUI、CoreBridge 权限边 | [architecture](architecture/README.md) |
