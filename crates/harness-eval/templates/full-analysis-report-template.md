...
> This report must be generated by an AI reviewer from the result package
> evidence. Do not fill it by copying only summaries. Use `report.json`,
> `execution-trace.json`, `provider-rounds/`, `tool-calls/`, and `evidence/`.
