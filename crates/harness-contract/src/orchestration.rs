...
pub struct ModelRoleIntent {
    ...
    #[serde(default)]
    #[schemars(
        description = "Requirements for one immutable Agent Definition, not a Team-wide union. Baseline valid profiles are [read], [read,search], [read,search,network], and [read,search,write,test]. Split network research from workspace mutation; no baseline profile combines network and write."
    )]
    pub required_capabilities: Vec<String>,
    #[serde(default)]
    #[schemars(
        description = "Exact immutable skill references already returned by runtime_capabilities(detail=agent_catalog). Omit for baseline Agents; never guess a skill name from the task domain."
    )]
    pub required_skills: Vec<String>,
    #[serde(default)]
    #[schemars(
        description = "Normally omit. Runtime derives physical tools from required_capabilities; include only a currently exposed exact native tool id when it is semantically mandatory."
    )]
