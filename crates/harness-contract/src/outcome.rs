...
mod delivery_contract_tests {
    ...
    fn runtime_build_identity_is_additive_flat_and_legacy_safe() {
        ...
        let current = RuntimeIdentity {
            ...
            runtime_revision: "0.9.687".to_string(),
            config_revision: "config-2".to_string(),
            build: RuntimeBuildIdentity::new("0.9.687", "a".repeat(40), false),
        };
        let value = serde_json::to_value(current).expect("RuntimeIdentity serializes");
        assert_eq!(value["semver"], "0.9.687");
    ...
    fn unresolved_source_can_only_be_recorded_as_dirty() {
        assert!(RuntimeBuildIdentity::unresolved_development("0.9.687")
            .validate_for_recording()
            .is_ok());
        assert!(RuntimeBuildIdentity::new("0.9.687", "unknown", false)
            .validate_for_recording()
            .is_err());
        assert!(RuntimeBuildIdentity::new("0.9.687", "b".repeat(40), false)
            .validate_for_recording()
            .is_ok());
    }
