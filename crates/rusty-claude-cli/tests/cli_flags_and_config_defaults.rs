...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    fs::write(temp_dir.join(".cowd.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".cowd").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    assert!(stdout.contains(temp_dir.join(".cowd.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".cowd")
