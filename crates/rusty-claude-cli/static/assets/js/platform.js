...
const Platform = {
    ...
    this.container.innerHTML = platforms.map(platform => `
      <div class="platform-card" data-platform-id="${platform.id}">
        ...
        <div class="platform-actions">
          ${platform.connected
            ? `<button class="btn secondary" onclick="window.Platform.disconnect('${platform.id}')">${window.i18nInstance?.t('platform.disconnect')}</button>`
            : `<button class="btn primary" onclick="window.Platform.connect('${platform.id}')">${window.i18nInstance?.t('platform.connect')}</button>`
          }
          <button class="btn secondary" onclick="window.Platform.showSettings('${platform.id}')">${window.i18nInstance?.t('platform.settings')}</button>
        </div>
      </div>
    `).join('');
  },
  ...
  async connect(platformId) {
    try {
      await window.api?.updatePlatform(platformId, { connected: true });
      window.Toast?.success('平台连接成功');
      this.loadPlatforms();
    } catch (error) {
      window.Toast?.error('连接失败');
    }
  },

  async disconnect(platformId) {
    try {
      await window.api?.updatePlatform(platformId, { connected: false });
      window.Toast?.success('平台已断开');
      this.loadPlatforms();
    } catch (error) {
      window.Toast?.error('断开失败');
    }
  },
