...
const Config = {
  ...
  renderConfig(config) {
    ...
    this.container.innerHTML = `
      ...
      <div class="config-section">
        <h3>${window.i18nInstance?.t('config.api')}</h3>

        <div class="config-item">
          <div>
            <label>${window.i18nInstance?.t('config.provider')}</label>
        ...
        <div class="config-item">
          <div>
            <label>${window.i18nInstance?.t('config.model')}</label>
      ...
      <div class="config-section">
        ...
        <div class="config-item">
          <div>
            <label>${window.i18nInstance?.t('config.theme')}</label>
            <p class="description">选择界面主题</p>
          </div>
          <select id="configTheme">
            <option value="dark" ${window.ThemeManager.getTheme() === 'dark' ? 'selected' : ''}>${window.i18nInstance?.t('config.theme.dark')}</option>
            <option value="light" ${window.ThemeManager.getTheme() === 'light' ? 'selected' : ''}>${window.i18nInstance?.t('config.theme.light')}</option>
            <option value="slate" ${window.ThemeManager.getTheme() === 'slate' ? 'selected' : ''}>${window.i18nInstance?.t('config.theme.slate')}</option>
          </select>
        </div>

        <div class="config-item">
          <div>
            <label>${window.i18nInstance?.t('config.language')}</label>
            <p class="description">选择界面语言</p>
          </div>
          <select id="configLanguage">
            <option value="zh-CN" ${window.i18nInstance?.getCurrentLang() || "zh-CN" === 'zh-CN' ? 'selected' : ''}>中文</option>
            <option value="en" ${window.i18nInstance?.getCurrentLang() || "zh-CN" === 'en' ? 'selected' : ''}>English</option>
          </select>
        </div>
      </div>
      ...
      <div class="config-section">
        <button class="btn primary" id="saveConfigBtn">
          ...
          ${window.i18nInstance?.t('config.save')}
        </button>
      </div>
    `;
  ...
  bindHandlers() {
    ...
    if (langSelect) {
      langSelect.addEventListener('change', (e) => {
        window.i18nInstance?.setLocale(e.target.value);
  ...
  async saveConfig() {
    ...
    try {
      ...
      const _t_cfg = (k,fb) => window.i18nInstance?.t(k) || fb; window.Toast?.success(t('config.saved'));
    } catch (error) {
