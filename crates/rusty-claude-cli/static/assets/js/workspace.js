...
class WorkspaceManager {
  ...
  async loadWorkspaces() {
    try {
      this.workspaces = await window.api?.listWorkspaces();
      window.appState?.set('workspaces', this.workspaces);
  ...
  async getCurrentWorkspace() {
    try {
      this.currentWorkspace = await window.api?.getWorkspace();
      window.appState?.set('workspace', this.currentWorkspace);
  ...
  async switchWorkspace(workspaceId) {
    try {
      const workspace = await window.api?.setWorkspace(workspaceId);
      this.currentWorkspace = workspace;
      window.appState?.set('workspace', workspace);

      // Refresh file tree
      await this.loadFileTree();

      window.Toast?.success(`已切换到工作区: ${workspace.name}`);
      return workspace;
    } catch (e) {
      window.Toast?.error(`切换工作区失败: ${e.message}`);
      return null;
    }
  }
  ...
  async createWorkspace(data) {
    try {
      const workspace = await window.api?.request('/workspaces', {
        method: 'POST',
        body: JSON.stringify(data),
      });

      this.workspaces.push(workspace);
      window.Toast?.success(`已创建工作区: ${workspace.name}`);
      return workspace;
    } catch (e) {
      window.Toast?.error(`创建工作区失败: ${e.message}`);
      return null;
    }
  }
  ...
  async deleteWorkspace(workspaceId) {
    try {
      await window.api?.request(`/workspaces/${workspaceId}`, {
        method: 'DELETE',
      });

      this.workspaces = this.workspaces.filter(w => w.id !== workspaceId);
      window.Toast?.success('工作区已删除');
    } catch (e) {
      window.Toast?.error(`删除工作区失败: ${e.message}`);
    }
  }
  ...
  async loadFileTree(path = '') {
    try {
      ...
      window.appState?.set('fileTree', this.fileTree);
  ...
  async createFile(data) {
    try {
      await window.api?.request('/workspace/files', {
        method: 'POST',
        body: JSON.stringify(data),
      });
      await this.loadFileTree();
      window.Toast?.success(`已创建文件: ${data.name}`);
    } catch (e) {
      window.Toast?.error(`创建文件失败: ${e.message}`);
    }
  }
  ...
  async deleteFile(path) {
    ...
    try {
      await window.api?.request(`/workspace/files?path=${encodeURIComponent(path)}`, {
        method: 'DELETE',
      });
      await this.loadFileTree();
      window.Toast?.success('文件已删除');
    } catch (e) {
      window.Toast?.error(`删除文件失败: ${e.message}`);
    }
  }
  ...
  async renameFile(oldPath, newPath) {
    try {
      await window.api?.request('/workspace/files/rename', {
        method: 'PUT',
        body: JSON.stringify({ oldPath, newPath }),
      });
      await this.loadFileTree();
      window.Toast?.success('文件已重命名');
    } catch (e) {
