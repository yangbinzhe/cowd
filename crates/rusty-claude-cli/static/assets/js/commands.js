...
class CommandManager {
  ...
  displayResult(result) {
    ...
    if (result.result) {
      ...
      window.messageRenderer?.addMessage(message);
    }
  }
  ...
  async cmdNewSession(args) {
    ...
    if (window.app) await window.app.loadSessions();
    return `已创建新会话: ${title}`;
  }
  ...
  async cmdMemory(args) {
    ...
    if (query) {
      const results = await window.api?.searchMemory(query);
    ...
    if (layer === 'all') {
      ...
      return memory ? Object.entries(memory)
        .map(([l, items]) => `[${l}] ${items.length} 条`)
        .join('\n') : '暂无记忆';
    }

    const items = await window.api?.getMemoryLayer(layer);
    return items ? items.map(i => `• ${i.content}`).join('\n') : '暂无数据';
  }
  ...
  async cmdRemember(args) {
    ...
    await window.api?.addMemory('working', { content });
    return '已添加到工作记忆';
  }
  ...
  async cmdSetConfig(args) {
    ...
    await window.api?.updateConfig({ [key]: value });
  ...
  async cmdConnect(args) {
    ...
    window.panelManager?.show('platform');
  ...
  async cmdDisconnect(args) {
    ...
    await window.api?.disconnectPlatform(platform);
    return `已断开 ${platform}`;
  }
  ...
  async cmdWorkspace(args) {
    ...
    if (workspace) {
      ...
      if (target) {
        await window.api?.setWorkspace(target.id);
        window.appState?.set('workspace', target);
        return `已切换到工作区: ${target.name}`;
      }
      return '工作区不存在';
    }
    const current = await window.api?.getWorkspace();
  ...
  async cmdExport(args) {
    ...
    if (type === 'all' || type === 'sessions') {
      data.sessions = await window.api?.listSessions();
    }
    if (type === 'all' || type === 'memory') {
      data.memory = await window.api?.getMemory();
    }
    if (type === 'all' || type === 'config') {
      data.config = await window.api?.getConfig();
    }
