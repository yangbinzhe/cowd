...
pub trait LifecycleHook: Send + Sync {
    async fn on_turn_start(&self, _user_input: &str) -> Result<(), String> { Ok(()) }
    async fn pre_tool_call(&self, _tool_name: &str, _input: &str) -> Result<(), String> { Ok(()) }
    async fn post_tool_call(&self, _tool_name: &str, _result: &str, _is_error: bool) -> Result<(), String> { Ok(()) }
    async fn on_turn_end(&self, _summary: &str) -> Result<(), String> { Ok(()) }
    async fn on_session_start(&self, _session_id: &str) -> Result<(), String> { Ok(()) }
    async fn on_session_end(&self, _session_id: &str) -> Result<(), String> { Ok(()) }
}
...
impl HookRunner {
    pub fn new() -> Self { Self { hooks: Vec::new(), bus: None } }
    pub fn register(&mut self, hook: Box<dyn LifecycleHook>) { self.hooks.push(hook); }

    pub async fn fire_turn_start(&self, input: &str) {
        for hook in &self.hooks { let _ = hook.on_turn_start(input).await; }
    }
    ...
    pub async fn fire_turn_end(&self, summary: &str) {
        for hook in &self.hooks { let _ = hook.on_turn_end(summary).await; }
    }
}
