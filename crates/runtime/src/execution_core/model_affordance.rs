...
pub fn runtime_execution_guidance_prompt_with_tool_exposure(
    ...
    format!(
        "## Runtime execution decision\nrecommended_pattern={}; evidence_mode={:?}; complexity={:?}; risk={:?}\ntemplate_selection=model_authored; runtime_never_falls_back\nrecommended_actions={}\naction_selection={}\nContract instruction: {}\n{}\nGuidance: simple work should be answered directly. When the right pattern is unclear, use `runtime_capabilities` once. Call `runtime_orchestrate` only through inspect/propose/propose_template/revise/control when its native schema is active. If the user expressly identifies a Team, role, responsibility, or organizational relationship and `submit_collaboration_decision` is active, use that tool only: preserve every user-provided identifier verbatim in its one-workstream-per-Team, turn-scoped `template`. Do not encode that custom Team again as a generic `runtime_orchestrate` proposal. The template must contain the complete role topology, typed behavior, directed dependencies, result fields and instructions; Runtime freezes it without catalog publication or a template-approval wait. Use a catalog template through `runtime_orchestrate` only when the user expressly selects that catalog template. Use `propose_template` only when the user expressly asks to publish/reuse a template. If a required custom definition is absent or invalid, report the structured correction and retry; never replace it with a builtin template. Proposals contain semantic recipes and dependencies, never executors, leases, system paths, or physical graph ids. Prefer independent parallel nodes, Team templates, review and synthesis over repeated serial probing. During Team work, publish bounded findings or conflicts to `team_board` and read after the last observed revision at safe checkpoints. If progress is useful but slow, continue with staged synthesis; if evidence novelty falls, revise the graph instead of repeating an unchanged path.",
