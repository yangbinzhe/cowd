...
pub(super) fn task_objective(
    ...
    match mode {
        DispatchMode::Execute => format!(
            "You are Agent `{}` in Team `{}`. Role: {}. Mission: {}.\n\nWork item `{}`: {}\nObjective: {}\nAcceptance: {}\n\nFirst call state_inspect and inspect the current Program truth. If this work fits your role and capabilities, actively call task_claim for this exact task before doing substantive work; Runtime binds that claim to your immutable Agent identity and physical execution. If it is unsuitable or already owned, do not claim or submit it: explain the mismatch concisely and let the Team reassign or replan. After a successful claim, act autonomously: choose and use the most effective available tools, and publish useful findings to topic:{} when collaboration benefits. Produce the substantive long-form work in ordinary content or a workspace artifact, then call artifact_commit with this exact task ref in relates_to. Finally call task_submit: pass the collaboration `artifact:...` value returned in artifact_commit.changed_refs as artifact_refs, and pass real durable source/test/tool receipts as evidence_refs. Runtime automatically binds the artifact's content; do not copy its internal artifact:// content_ref into evidence_refs. Never submit before claiming, and never claim completion only in prose.",
            member.agent_id,
            task.team_id,
            member.role,
            member.mission,
