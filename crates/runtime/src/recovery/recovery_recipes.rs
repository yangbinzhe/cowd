...
impl std::fmt::Display for FailureScenario {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ContextWindowBlocked => write!(f, "context_window_blocked"),
        }
    }
}
...
pub enum RecoveryStep {
    ...
    RestartWorker,
    CompactSession,
    TruncateOldestTurns { keep_recent: usize },
...
pub fn recipe_for(scenario: &FailureScenario) -> RecoveryRecipe {
    match scenario {
        ...
        FailureScenario::ContextWindowBlocked => RecoveryRecipe {
            scenario: *scenario,
            steps: vec![
                RecoveryStep::CompactSession,
                RecoveryStep::CompactSession, // deep compact as second attempt
            ],
            max_attempts: 2,
            escalation_policy: EscalationPolicy::LogAndContinue,
        },
