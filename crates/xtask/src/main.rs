...
fn render_product_catalogue(locks: &[AppSourceLock]) -> String {
    let registrations = locks
        ...
        .map(|lock| {
            format!(
                "        #[cfg(feature = \"{}\")]\n        {}::product().with_source_lock(cowd_app_host::StaticAppSourceLock::new(\n            {:?},\n            {:?},\n        )),",
                app_feature(lock),
