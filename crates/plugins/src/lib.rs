...
/// Legacy manifest path used by older bundled plugins (pre-cowd migration).
...
fn plugin_manifest_path(root: &Path) -> Result<PathBuf, PluginError> {
    ...
    // Backward compatibility: older plugins used .claude-plugin/plugin.json
...
/// Guards against concurrent modification of `COWD_CONFIG_HOME`.
#[cfg(test)]
...
mod tests {
  ...
  "description": "Packaged manifest in install root"
    ...
    fn claw_config_home_isolation_prevents_host_plugin_leakage() {
        ...
        // Set COWD_CONFIG_HOME to our temp directory
  ...
  "description": "Test fixture plugin in isolated config home"
        ...
        // Create PluginManager with isolated bundled_root - it should use the temp config_home, not host ~/.cowd/
        let mut config = PluginManagerConfig::new(&config_home);
        config.bundled_root = Some(bundled_root.clone());
        let manager = PluginManager::new(config);
        ...
        assert_eq!(
            installed.len(),
            1,
            "should only see the test fixture plugin, not host ~/.cowd/plugins/"
        );
        assert_eq!(
