...
#[test]
fn workspace_snapshot_discovers_real_roots_and_every_continuation_is_executable() {
    ...
    fs::write(root.join("MFG/.git/HEAD"), "ref: refs/heads/mfg-source\n").unwrap();
    fs::write(root.join("MFG/Cargo.toml"), "[workspace]\n").unwrap();
...
#[test]
fn workspace_snapshot_observes_ignore_overrides_and_external_changes() {
    let root = temp_path("workspace-discovery-ignore");
