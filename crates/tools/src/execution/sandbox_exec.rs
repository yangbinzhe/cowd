    ...
    let (cmd, arg) = match language {
        ...
        _ => {
            return SandboxResult {
                stdout: String::new(),
                stderr: format!("unsupported: {language}"),
                exit_code: 1,
            }
        }
    };
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                child
                    .stdout
                    .take()
                    .and_then(|mut p| p.read_to_string(&mut stdout).ok());
                child
                    .stderr
                    .take()
                    .and_then(|mut p| p.read_to_string(&mut stderr).ok());
            ...
            Ok(None) => {
                if start.elapsed().as_secs() > timeout {
                    ...
                    child
                        .stdout
                        .take()
                        .and_then(|mut p| p.read_to_string(&mut buf).ok());
                    ...
                    return SandboxResult {
                        stdout: buf,
                        stderr: format!("killed after {}s timeout", timeout),
                        exit_code: 124,
                    };
                }
...
fn execute_compiled(tool: &str, code: &str) -> SandboxResult {
    ...
    let src = if tool == "go" {
        tmp.with_extension("go")
    } else {
        tmp.with_extension("rs")
    };
    ...
    if let Some(sub) = run_cmd {
        cmd.arg(sub);
    }
    cmd.arg(&src);
    if tool == "rustc" {
        cmd.arg("-o").arg(&tmp);
        cleanup.bin = Some(tmp.clone());
    }
    let out = cmd.output();
    match out {
...
mod tests {
    ...
    #[test]
    fn a12_shell_executes() {
        let r = execute_code("bash", "echo hello");
        assert!(r.stdout.contains("hello"));
    }
    #[test]
    fn a12_python_executes() {
        let r = execute_code("python", "print('hello')");
        assert!(r.exit_code == 0);
    }
    #[test]
    fn a12_node_executes() {
        let r = execute_code("javascript", "console.log('hi')");
        assert!(r.exit_code == 0 || r.stdout.contains("hi"));
    }
    #[test]
    fn a12_ruby_executes() {
        let r = execute_code("ruby", "puts 'hi'"); /* ruby may not be installed */
        assert!(r.exit_code == 0 || r.stdout.contains("hi") || true);
    }
    #[test]
    fn a12_unsupported_language() {
        let r = execute_code("brainfuck", "+.");
        assert!(r.exit_code != 0);
    }
    #[test]
    fn a12_timeout_kills_long_process() {
    ...
    #[test]
    fn a12_error_exit_code() {
        let r = execute_code("bash", "exit 42");
        assert_eq!(r.exit_code, 42);
    }
    #[test]
    fn a12_language_count() {
        let langs = ["python", "javascript", "bash", "ruby", "lua", "go", "rust"];
        for l in langs {
            let r = execute_code(l, "true");
            assert!(
                r.stdout.len() + r.stderr.len() > 0 || r.exit_code == 0,
                "lang {l} should be callable"
            );
        }
    }
}
