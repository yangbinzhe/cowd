...
pub fn handle_handoff_command(
    ...
    match action {
        "save" | "create" => {
            ...
            Ok(format!(
                "Handoff data for session '{}' would be saved here.\n\
                 To save current session state, use /handoff save <session-id>\n\
                 Handoff directory: {}",
                sid,
                handoff_dir.display()
            ))
        ...
        "load" | "resume" => {
            ...
            match manager.load(sid) {
                ...
                Ok(None) => Ok(format!("No handoff found for session '{}'", sid)),
...
/// JSON version of handoff command for programmatic access
pub fn handle_handoff_command_json(
    action: Option<String>,
    session_id: Option<String>,
    cwd: &Path,
    ...
    match action {
        ...
        "load" | "resume" => {
            ...
            match manager.load(sid) {
                ...
                Ok(None) => Ok(serde_json::json!({
                    "success": false,
                    "message": format!("No handoff found for session '{}'", sid)
                })),
                Err(e) => Ok(serde_json::json!({
                    "success": false,
                    "error": e.to_string()
        ...
        _ => Ok(serde_json::json!({
            "success": false,
            "message": format!("Unknown action: {}. Use: list, load, save, latest, help", action)
        })),
    }
}
...
pub fn create_session_handoff(
    ...
    Ok(format!(
        ...
        session_id,
        handoff_dir.join(format!("{}.md", session_id)).display()
    ))
}
...
pub(crate) fn generate_session_id() -> String {
    let timestamp = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    format!("handoff-{}", timestamp)
}

fn render_handoff_data(data: &HandoffData) -> String {
...
Usage:
  /handoff save [session-id]   Save current session state
  /handoff load <session-id>  Load a saved handoff
  /handoff list               Show latest handoff
  /handoff latest             Show full latest handoff
  /handoff help               Show this help

Aliases: /transfer, /handover

Examples:
  /handoff save my-task       Save current state with ID 'my-task'
  /handoff load my-task       Load handoff 'my-task'
...
mod handoff_tests {
    ...
    fn test_generate_session_id() {
        ...
        assert!(id1.starts_with("handoff-"));
    }

    #[test]
    fn test_render_handoff_help() {
        let help = render_handoff_help();
...
mod tests {
    ...
    fn test_generate_session_id() {
        let id1 = generate_session_id();
        assert!(id1.starts_with("handoff-"));
        // IDs should be unique-ish (based on timestamp)
