...
fn clean_env_cli_reaches_mock_anthropic_service_across_scripted_parity_scenarios() {
    ...
    for case in cases {
        // Skip plugin_tool_roundtrip: external plugin loading is not yet wired
        // into the test runtime; the tool returns "unsupported tool" instead of
        // executing the plugin script.
        if case.name == "plugin_tool_roundtrip" {
            eprintln!("SKIP: plugin_tool_roundtrip (external plugins not wired in test runtime)");
            ...
            scenario_reports.push(ScenarioReport {
                ...
                tool_uses: Vec::new(),
                tool_error_count: 0,
                final_message: "SKIPPED".to_string(),
            });
            continue;
        }
    ...
    assert_eq!(
        messages_only.len(),
        19,
        "scenarios should produce nineteen /v1/messages requests (total captured: {}, includes count_tokens)",
        captured.len()
    );
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("COWD_CONFIG_HOME", &workspace.config_home)
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("cowd should finish")
    } else {
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    let manifest_dir = plugin_root.join(".cowd/plugins");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "cowd-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
