...
macro_rules! tui_test {
    ($name:ident, $body:expr) => {
        #[test]
        fn $name() {
            $crate::test_utils::__tui_test_setup($body);
        }
    };
}
...
mod tests {
    ...
    fn fixture_roundtrip() {
        ...
        for (i, entry) in app.timeline_iter() {
            ...
            match entry {
                crate::app::TimelineEntry::Message { role, content, .. } => {
    ...
    fn fixture_messages_alternate_roles() {
        ...
        let roles: Vec<&str> = app
            ...
            .filter_map(|(_, e)| match e {
                crate::app::TimelineEntry::Message { role, .. } => Some(role.as_str()),
                _ => None,
            })
            .collect();
    ...
    fn fixture_app_streaming_is_active() {
        ...
        let msg = app
            ...
            .find_map(|(_, e)| match e {
                crate::app::TimelineEntry::Message { content, .. } => Some(content.clone()),
                _ => None,
            })
