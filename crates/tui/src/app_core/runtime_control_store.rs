...
pub(crate) mod tests {
    ...
    #[test]
    fn managed_surface_is_external_without_legacy_entry() {
        ...
        let surface = surface_summary_from_json(&value).expect("managed surface summary");
    ...
    #[test]
    fn missing_surface_transport_does_not_claim_legacy_stdio() {
        let value = serde_json::json!({
            "id": "surface:future",
            "lifecycle": "managed"
        });
        let surface = surface_summary_from_json(&value).expect("future surface summary");

        assert_eq!(surface.transport, "unknown");
    }
