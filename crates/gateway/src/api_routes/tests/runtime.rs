...
async fn list_sessions_filters_and_paginates_unified_store() {
    ...
    other_model.metadata_json = Some(serde_json::json!({"title":"Auth Other Model"}).to_string());
...
async fn delete_running_session_cancels_and_drains_before_committing_tombstone() {
    ...
    assert!(
        !runtime.has_active_turn_for_session(session_id),
        "lifecycle drain is complete when the process-owned turn guard exits"
    );
    assert!(
        !runtime
            .execution_live(execution_id)
            .expect("delete keeps auditable live diagnostics")
            .status
            .is_terminal(),
        "process shutdown must not fabricate a cancellation terminal winner"
    );
    assert_eq!(
        store
            .get_session(session_id)
...
#[tokio::test]
async fn runtime_projection_uses_selected_empty_pg_sources() {
    ...
    assert_ne!(json["health_summary"]["status"], "degraded");
    assert_eq!(json["health_summary"]["degraded_events"], 0);
    assert!(json["health_summary"]["reasons"]
        .as_array()
        .is_some_and(|reasons| reasons.iter().all(|reason| reason != "session store not available")));
    assert_ne!(json["value_loop"]["status"], "degraded");
    assert_eq!(json["value_loop"]["missing_required_count"], 7);
    assert_eq!(json["agent_value"]["status"], "unproven");
    assert_eq!(
