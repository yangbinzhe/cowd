...
#[tokio::test]
async fn agentic_program_route_returns_only_the_typed_root_program() {
    use harness_contract::agent_action::{
        program_id_for_objective, root_objective_id, AgentAction, AgentActionEnvelope,
        AgentActorBinding, AgentActorKind, TeamCreateInput,
    };
    ...
    let observed = runtime
        ...
        .apply(&AgentActionEnvelope {
            ...
            actor: AgentActorBinding {
                ...
                root_execution_id: Some("agentic-program-route-root".to_string()),
                required_team_count: 1,
                objective_summary: "typed route projection".to_string(),
                model_lease: "test-model".to_string(),
                permission_ceiling: Some(harness_contract::policy::PermissionMode::ReadOnly),
                resource_scopes: vec!["read:crates".to_string()],
                actor_id: "root-agent".to_string(),
            ...
            action: AgentAction::TeamCreate(TeamCreateInput {
                ...
                mission: "read evidence".to_string(),
                objective: None,
            }),
        })
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/runtime/agentic/programs/root?session_id={session_id}&turn_id={turn_id}"
                ))
                .body(Body::empty())
                .unwrap(),
        )
    ...
    assert!(body.get("graph").is_none(), "legacy graph must not leak");

    let restricted = AuthenticatedPrincipal(runtime::VerifiedPrincipal::from_test_claims(
        harness_contract::security::PrincipalClaims {
            principal_id: "unrelated-reader".to_string(),
            tenant_id: "tenant:test".to_string(),
            grant_id: "grant:unrelated-reader".to_string(),
    ...
    let forbidden = runtime_routes::get_root_agentic_program(
        axum::extract::State(Arc::clone(&state)),
        Extension(restricted),
        axum::extract::Query(runtime_routes::AgenticProgramRootQuery {
            session_id: session_id.to_string(),
            turn_id: turn_id.to_string(),
        }),
    )
    .await
    .expect_err("unrelated principal must not read the Program");
    ...
    let missing = app
            ...
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/runtime/agentic/programs/root?session_id={session_id}&turn_id=missing-turn"
                    ))
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    let oversized = app
            ...
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/runtime/agentic/programs/root?session_id={oversized_session}&turn_id={turn_id}"
                    ))
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    let unknown_query_field = app
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/runtime/agentic/programs/root?session_id={session_id}&turn_id={turn_id}&program_id=forged"
                    ))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
    assert_eq!(unknown_query_field.status(), StatusCode::BAD_REQUEST);
}
...
async fn mission_control_route_exposes_runtime_projection_and_command_router() {
    ...
    let control: serde_json::Value =
        serde_json::from_slice(&to_bytes(control.into_body(), usize::MAX).await.unwrap()).unwrap();
    assert_eq!(
...
#[tokio::test]
async fn mission_routes_reject_legacy_team_mutation_write_approvals_and_project_proxies() {
    let _guard = mission_route_lock().lock().await;
    ...
    assert_eq!(team_status, StatusCode::BAD_REQUEST);
    let team_json: serde_json::Value = serde_json::from_slice(&team_body).unwrap();
    assert!(team_json["error"]
        .as_str()
        .is_some_and(|error| error.contains("not a Mission-owned command target")));
    ...
    let approval_json: serde_json::Value =
        serde_json::from_slice(&to_bytes(approval.into_body(), usize::MAX).await.unwrap()).unwrap();
    ...
    let proxy_json: serde_json::Value =
        serde_json::from_slice(&to_bytes(proxy.into_body(), usize::MAX).await.unwrap()).unwrap();
...
async fn mission_control_keeps_unbound_session_out_of_default_mission() {
    ...
    let events_json: serde_json::Value =
        serde_json::from_slice(&to_bytes(events.into_body(), usize::MAX).await.unwrap()).unwrap();
...
async fn cowd_release_gate_route_reports_missing_timeline_evidence() {
    ...
    assert!(json["checks"].as_array().unwrap().iter().any(|check| {
        check["check_id"] == "structured_data.memory_context.bridge" && check["status"] == "fail"
    }));
}
    ...
    assert!(outcome_events.iter().any(|event| {
        event["refs"].as_array().is_some_and(|refs| {
            refs.iter().any(|reference| {
                reference["type"] == "structured_fact" && reference["id"] == "fact-outcome-stock"
            })
        })
    }));
    assert!(outcome_events.iter().any(|event| {
