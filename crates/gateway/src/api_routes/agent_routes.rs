...
async fn agent_catalog_handler(
    ...
    command_service::handle_agents_slash_command_json(Some("list"), &state.workspace_root)
        .map(Json)
...
async fn agent_discover_handler(
    ...
    command_service::handle_agents_slash_command_json(
...
async fn agent_assemble_handler(
    ...
    let discovery = command_service::handle_agents_slash_command_json(
        Some(&format!("discover {task}")),
        &state.workspace_root,
    )
    .map_err(|error| api_error(StatusCode::INTERNAL_SERVER_ERROR, error.to_string()))?;
...
async fn agent_team_profile_create_handler(
    ...
    if profiles.iter().any(|existing| existing.id == profile.id) {
        return Err(api_error(
            StatusCode::CONFLICT,
            "team profile id already exists",
        ));
    }
