...
use crate::gateway_services::RuntimeContextBoundary;
    ...
    if let Some(runtime_entry) = state.active_runtime(&session_id) {
        if let Ok(runtime) = runtime_entry.try_lock() {
            if let Some(envelope) = runtime.last_context_envelope() {
                ...
                let cache_stability =
                    RuntimeContextBoundary::cache_stability_report(&envelope, &envelope);
...
fn resource_context_items(state: &AppState, query: &str) -> Vec<ContextItem> {
    ...
    let resources = if query.trim().is_empty() {
        state
            .services
            .connector
            .recent_resources(&state.workspace_root, 5)
    } else {
        state
            .services
            .connector
            .search_resources(&state.workspace_root, query, 5)
    }
    .unwrap_or_default();
