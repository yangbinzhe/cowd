...
use runtime::{
    ...
    McpOAuthConfig, McpServerConfig, MessageRole, PermissionMode, PermissionPolicy, ProjectContext,
    PromptCacheEvent, ResolvedPermissionMode, ResumeContextPacket, ResumeContextSource,
    RuntimeError, ScopedMcpServerConfig, Session, TokenUsage, ToolError, ToolExecutor,
    UsageTracker,
};
...
fn print_static_config_command(
    args: Option<&str>,
    output_format: CliOutputFormat,
) -> Result<(), Box<dyn std::error::Error>> {
...
fn run_tui_repl(mut cli: LiveCli, workspace: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (runtime_event_tx, runtime_event_rx) =
        std::sync::mpsc::sync_channel::<runtime::CowdEvent>(256);
    ...
    let mut gateway_session_ids: Vec<String> = Vec::new();
    let mut gateway_session_attached = false;
    let gateway_actor_id = format!("tui:{}", std::process::id());
    let mut gateway_lease_owner: Option<String> = None;
    let gateway_client = match tui::gateway_client::GatewayApiClient::ensure_running_with_retry(
        tui::gateway_client::default_auth_token(),
    ) {
        Ok(client) => client,
        Err(err) => {
            state.add_message("system", &format!("Gateway API client unavailable: {err}"));
            None
        }
    };
    if let Some(gateway_client) = gateway_client.as_ref() {
        match SHARED_RT.block_on(gateway_client.status()) {
            Ok(status) => {
                ...
                state.add_message(
                    "system",
                    &format!(
                        "Gateway API connected: {} active sessions, uptime {}s",
                        active_api_sessions, server_uptime_secs
                    ),
                );
                match SHARED_RT.block_on(gateway_client.ensure_session(&session_id, &cli.model)) {
                    Ok(ensured) => {
                        ...
                        match SHARED_RT.block_on(gateway_client.attach_session(
                            ...
                            Ok(attached) => {
                                ...
                                match SHARED_RT.block_on(gateway_client.replay_session(
                                    &ensured_session_id,
                                    0,
                                    100,
                                )) {
                        ...
                        let lease_owner = gateway_actor_id.clone();
                    ...
                    Err(err) => {
                        state.add_message(
                            "system",
                            &format!(
                            "Gateway session attach failed; local runtime remains active: {err}"
                        ),
                        );
                    }
                }
                ...
                gateway_session_ids = snapshot.session_ids.clone();
                ...
                if let Some(readiness) = readiness {
                    ...
                    "system",
                    &format!(
                        "Gateway runtime projection connected: readiness={readiness}, components={components}"
                    ),
                );
                }
                ...
                if gateway_session_attached {
                    ...
                    let _event_bridge = SHARED_RT.spawn(async move {
                        if let Err(err) = event_client
                            ...
                            .await
                        {
                            let _ = event_tx.send(tui::CowdEvent::TurnError {
            ...
            Err(err) => {
                ...
                state.add_message(
                    "system",
                    "Gateway API unavailable; local TUI runtime active.",
                );
            }
        }
    } else {
    ...
    tracing::debug!("tui init: wiring memory projection");
    state.set_memory_projection_available(cli.runtime.memory_manager().is_some());

    // ── Wire tool registry to SkillsPanel (T27) ──
    tracing::debug!("tui init: loading tool registry");
    if let Ok(registry) = runtime_bootstrap::load_tool_registry_for_current_dir() {
    ...
    // ── Populate TUI session list from the unified SQLite store ──
    {
        ...
        let _ = tui_tx.send(tui::CowdEvent::SessionList {
        ...
        loop {
            tokio::select! {
                        ...
                        if key.kind == KeyEventKind::Press {
                            ...
                            match state.process_raw_key(key) {
                                ProcessedKey::Submit(text) => {
                                    ...
                                    if gateway_session_attached {
                                        let Some(event_client) = gateway_client.clone() else {
                                            state.add_message(
                                                "system",
                                        ...
                                        SHARED_RT.spawn(async move {
                                            match event_client
                                                .send_message(&event_session_id, &text)
                                                .await
                                            {
                                                Ok(value) => {
                                                    ...
                                                    {
                                                        let _ = event_tx.send(
                                                            tui::CowdEvent::TextDelta {
                                                                text: response.to_string(),
                                                            },
                                                        );
                                                    }
                                                    let _ = event_tx
                                                        .send(tui::CowdEvent::TurnComplete {
                                                ...
                                                Err(err) => {
                                                    let _ = event_tx.send(
                                                        tui::CowdEvent::TurnError {
                                                            error: format!(
                                                                "Gateway chat failed: {err}"
                                                            ),
                                                        },
                                                    );
                                                }
                                    ...
                                    let callback: std::sync::Arc<dyn runtime::ToolCallback> =
                                        std::sync::Arc::new(tui_callbacks::TuiToolCallback::new(
                                            tui_tx.clone(),
                                            cli.runtime.memory_manager().map(|mgr| mgr.orchestrator()),
                                        ));
                                    let runtime_stream_tx = runtime_event_tx.clone();
                                    let (mut prepared, monitor, abort_signal) =
                                        cli.prepare_turn_runtime(false, Some(callback), Some(runtime_stream_tx.clone()))?;
                                    prepared.set_memory_callback(
                                        std::sync::Arc::new(tui_callbacks::TuiMemoryCallback::new(tui_tx.clone())),
                                    );
                                    abort_monitor = Some(monitor);
                                    abort_signal_for_turn = Some(abort_signal);

                                    let tx = runtime_stream_tx;
                ...
                _ = tokio::time::sleep(Duration::from_millis(16)) => {
                    drain_runtime_cowd_events_state(&runtime_event_rx, &mut state);
    ...
    if let (Some(client), Some(owner)) = (gateway_client.as_ref(), gateway_lease_owner.as_deref()) {
        if let Err(err) =
            SHARED_RT.block_on(client.release_runtime_session_lease(&session_id, owner))
        {
            tracing::debug!(error = %err, session_id = %session_id, owner, "best-effort Gateway session lease release failed");
        }
    }
    if gateway_session_attached {
        if let Some(client) = gateway_client.as_ref() {
            if let Err(err) =
                SHARED_RT.block_on(client.detach_session(&session_id, &gateway_actor_id))
            {
                tracing::debug!(error = %err, session_id = %session_id, actor = %gateway_actor_id, "best-effort Gateway session lifecycle detach failed");
            }
        }
    }
    cli.persist_session()?;
...
fn drain_runtime_cowd_events_state(
    rx: &std::sync::mpsc::Receiver<runtime::CowdEvent>,
    state: &mut tui::state::TuiState,
) {
    let mut count = 0;
    let limit = if state.turn_active { 64 } else { 256 };
    while let Ok(event) = rx.try_recv() {
        if let Some(event) = runtime_event_to_tui_event(event) {
            state.apply_event(event);
        }
        count += 1;
        if count >= limit {
...
fn runtime_event_to_tui_event(event: runtime::CowdEvent) -> Option<tui::CowdEvent> {
    match event {
        runtime::CowdEvent::TextDelta { text } => Some(tui::CowdEvent::TextDelta { text }),
        runtime::CowdEvent::ThinkingDelta { thinking } => {
            Some(tui::CowdEvent::ThinkingDelta { thinking })
        }
        runtime::CowdEvent::ThinkingComplete => Some(tui::CowdEvent::ThinkingComplete),
        runtime::CowdEvent::SignatureDelta { signature } => {
            Some(tui::CowdEvent::SignatureDelta { signature })
        }
        runtime::CowdEvent::ToolStart { id, name, preview } => {
            Some(tui::CowdEvent::ToolStart { id, name, preview })
        }
        runtime::CowdEvent::ToolProgress { id, name, progress } => {
            Some(tui::CowdEvent::ToolProgress { id, name, progress })
        }
        runtime::CowdEvent::ToolComplete {
            ...
            summary,
            exit_code,
        } => Some(tui::CowdEvent::ToolComplete {
            id,
            name,
            summary,
            exit_code,
        }),
        runtime::CowdEvent::ToolExecuted { name, duration_ms } => {
            Some(tui::CowdEvent::ToolExecuted { name, duration_ms })
        }
        runtime::CowdEvent::TurnStarted => Some(tui::CowdEvent::TurnStarted),
        runtime::CowdEvent::TurnComplete {
            assistant_text,
            iterations,
        } => Some(tui::CowdEvent::TurnComplete {
            assistant_text,
            iterations,
        }),
        runtime::CowdEvent::TurnError { error } => Some(tui::CowdEvent::TurnError { error }),
        runtime::CowdEvent::ContextWindow(value) => Some(tui::CowdEvent::ContextWindow(value)),
        runtime::CowdEvent::ContextEnvelope { envelope } => Some(tui::CowdEvent::ContextEnvelope {
            envelope: serde_json::to_value(envelope).unwrap_or(serde_json::Value::Null),
        }),
        runtime::CowdEvent::RuntimePolicyDecision { summary } => {
            Some(tui::CowdEvent::RuntimePolicyDecision {
                summary: tui::RuntimePolicyDecisionSummary {
                    level: summary.level,
                    score: summary.score,
                    recommended_profile: summary.recommended_profile,
        ...
        runtime::CowdEvent::WorkGraphSummary { summary } => {
            Some(tui::CowdEvent::WorkGraphSummary {
                summary: tui::RuntimeWorkGraphSummary {
                    ...
                    synthesis_lift: summary.synthesis_lift,
                    complementarity_score: summary.complementarity_score,
                },
            })
        }
        runtime::CowdEvent::Warning { message } => Some(tui::CowdEvent::Warning { message }),
        runtime::CowdEvent::TokenUsage {
            input,
            output,
            cache_create,
            cache_read,
        } => Some(tui::CowdEvent::TokenUsage {
            input,
            output,
            cache_create,
            cache_read,
        }),
        runtime::CowdEvent::CompactionNotice { removed_count } => {
            Some(tui::CowdEvent::CompactionNotice { removed_count })
        }
        runtime::CowdEvent::SessionCreated { id, name } => {
            Some(tui::CowdEvent::SessionCreated { id, name })
        }
        runtime::CowdEvent::SessionDeleted { id } => Some(tui::CowdEvent::SessionDeleted { id }),
        runtime::CowdEvent::SessionSwitched { id, name } => {
            Some(tui::CowdEvent::SessionSwitched { id, name })
        }
        runtime::CowdEvent::SessionList { sessions } => {
            Some(tui::CowdEvent::SessionList { sessions })
        }
        runtime::CowdEvent::MemoryEntry {
            layer,
            content,
            relevance,
        } => Some(tui::CowdEvent::MemoryEntry {
            layer,
            content,
            relevance,
        }),
        runtime::CowdEvent::MemoryUpdate { entries, status } => {
            Some(tui::CowdEvent::MemoryUpdate { entries, status })
        }
        runtime::CowdEvent::MemoryStats {
            total_entries,
            vector_count,
            layers,
        } => Some(tui::CowdEvent::MemoryStats {
            total_entries,
            vector_count,
            layers,
        }),
        runtime::CowdEvent::MemoryExtracted { count } => {
            Some(tui::CowdEvent::MemoryExtracted { count })
        }
        runtime::CowdEvent::ApprovalRequested { tool } => {
            Some(tui::CowdEvent::ApprovalRequested { tool })
        }
    }
}
...
fn consume_session_sidebar_actions(state: &mut TuiState, cli: &mut LiveCli, workspace: &PathBuf) {
    ...
    if let Some(options) = state.pending_export_options.take() {
        ...
        state.toast_manager.push(
            tui::components::toast::ToastVariant::Success,
            Some("Export".into()),
...
#[derive(Debug, Clone, PartialEq, Eq)]
enum GatewayCrossPlaneSlashCommand {
    Summary,
    Preflight(String),
    Execute(String),
    Help,
...
fn parse_gateway_task_slash_command(args: Option<&str>) -> Result<GatewayTaskSlashCommand, String> {
...
fn parse_gateway_approval_slash_command(
    args: Option<&str>,
...
fn parse_gateway_context_slash_command(
    args: Option<&str>,
...
fn parse_gateway_cross_plane_slash_command(
    args: Option<&str>,
...
impl LiveCli {
    ...
    fn handle_gateway_tasks_command(
        &mut self,
        args: Option<&str>,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_gateway_approvals_command(
        &mut self,
        args: Option<&str>,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_gateway_context_command(
        &mut self,
        args: Option<&str>,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_gateway_cross_plane_command(
        &mut self,
        args: Option<&str>,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            SlashCommand::Help => {
                ...
                println!("**Gateway**: /tasks /approvals /context /cross-plane");
            ...
            SlashCommand::Tasks { args } => {
                self.handle_gateway_tasks_command(args.as_deref())?;
                false
            }
            SlashCommand::Approvals { args } => {
                self.handle_gateway_approvals_command(args.as_deref())?;
                false
            }
            SlashCommand::Context { action } => {
                self.handle_gateway_context_command(action.as_deref())?;
                false
            }
            SlashCommand::CrossPlane { args } => {
                self.handle_gateway_cross_plane_command(args.as_deref())?;
                false
            }
            SlashCommand::Stats => {
...
pub(crate) fn build_runtime(
    ...
    let runtime_plugin_state = runtime_bootstrap::assemble_runtime_state()?;
...
mod tests {
    ...
    use super::{
        activate_live_cli_session, build_runtime_with_plugin_state, build_system_prompt_for_mode,
        cli_turn_context_profile, collect_session_prompt_history, create_managed_session_handle,
        ...
        CliToolExecutor, GatewayAction, GatewayApprovalSlashCommand, GatewayContextSlashCommand,
        GatewayCrossPlaneSlashCommand, GatewayTaskSlashCommand, GitWorkspaceSummary, LiveCli,
        LocalHelpTopic, PromptHistoryEntry, SessionHandle, SlashCommand, StatusUsage,
...
"#,
        ...
        assert!(cli.session.path.ends_with("session.sqlite"));
