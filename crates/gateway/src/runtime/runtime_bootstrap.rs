    ...
    vec![
        ...
        RuntimeToolDefinition {
            name: harness_contract::orchestration::SUBMIT_COLLABORATION_DECISION_TOOL_ID
                .to_string(),
            description: Some(
                "Submit the initial semantic collaboration decision for this user turn. Use it for all user-named, turn-scoped Teams: one workstream is one Team and its optional template contains the complete arbitrary role topology, display names, typed behavior and directed role dependencies. Each role's required_capabilities must fit one runnable Agent profile, not union Team-wide abilities. Baseline profiles are [read], [read,search], [read,search,network], and [read,search,write,test]; split network research from workspace mutation and omit required_skills unless the exact reference came from runtime_capabilities(detail=agent_catalog). Runtime validates and freezes the snapshot without catalog publication. Runtime owns physical graph nodes, permissions, resources, approvals, recovery and terminal state. Do not use this for inspect, revision, catalog template publication, or control."
                    .to_string(),
            ),
