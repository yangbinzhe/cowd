...
fn agent_action_tool_definitions() -> Vec<RuntimeToolDefinition> {
    ...
    vec![
        agent_action_definition::<action::StateInspectInput>(
            ...
            "Inspect the current Program, Team, roster, work, topic and artifact facts. Runtime binds the Objective, Program and actor. Root: leave wait_for_workers=false to continue planning and publishing tasks while members execute; set it true only when ready to yield to workers until their work settles. Do not poll unchanged state. If you only need to know whether anything changed, pass after_revision with the revision you last saw: Runtime returns a tiny `{unchanged: true, revision}` receipt when nothing advanced, so repeated checks stay cheap. Optional scope_ref=collaboration_patterns returns evidence-backed structural suggestions from previous completed Turns; they are advice, not execution rules or capability grants.",
        ),
