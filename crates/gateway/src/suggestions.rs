...
use slash_contract::slash_command_specs;
use std::str;
...
pub(crate) fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `cowd --help` for CLI usage, or start `cowd` and use /help.");
    message
}
...
pub(crate) fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:").then_some(
        "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. Cowd does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
    )
}
