...
GENERATED_PROJECTION_GOLDEN="${WEBUI_ROOT}/src/generated/projection-golden.ts"
...
if [[ "${MODE}" == "update" ]]; then
  ...
  PROJECTION_GOLDEN_CANDIDATE="${SCENARIO_ROOT}/projection-golden.ts"
  ...
  if ! cmp -s "${GENERATED_PROJECTION_GOLDEN}" "${PROJECTION_GOLDEN_CANDIDATE}"; then
    echo "committed execution projection golden corpus is stale; run: bash scripts/scenarios/openapi-generation.sh update" >&2
    diff -u "${GENERATED_PROJECTION_GOLDEN}" "${PROJECTION_GOLDEN_CANDIDATE}" >&2 || true
    exit 1
  fi
