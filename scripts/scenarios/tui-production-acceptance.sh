...
for command in tmux curl node python3 rg ss pgrep getconf; do
...
# --- Legacy SQLite old-session acceptance (E3/E9/E10): RETIRED ---
# The retired fixture copied and directly tampered a `session.sqlite` database
# to prove old-session portability. Under the PostgreSQL-only release there is
# no supported SQLite session path and no historical database import, so these
# steps are retired instead of re-run against an unsupported store.
# PostgreSQL-native successors: scripts/test/postgres-contract.sh (session
# behavior conformance), scripts/scenarios/tui-interaction-quality.sh (TUI
# projection smoke), plus the Runtime/Gateway authority, draft-restoration,
# projection-revoke and transcript-isolation tests.
echo "SKIP: legacy SQLite old-session acceptance (E3/E9/E10) retired under PostgreSQL-only" >&2
