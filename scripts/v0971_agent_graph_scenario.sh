...
PORT="${COWD_V0971_PORT:-18691}"
BASE_URL="http://127.0.0.1:$PORT"
SESSION="cowd-v0971-agent-$$"
TMP_DIR="$(mktemp -d /tmp/cowd-v0971-agent.XXXXXX)"
...
curl -fsS "$BASE_URL/api/agents/runs" \
  | rg -q '"kind":"agent_run_graphs"'
curl -fsS "$BASE_URL/api/agents/runs" \
  | rg -q 'agent graph visible'
curl -fsS "$BASE_URL/api/tasks/$task_id/agent-graph" \
  | rg -q 'accepted by agent graph scenario'
