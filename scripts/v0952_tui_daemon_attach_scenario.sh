...
PORT="${COWD_V0952_PORT:-18672}"
BASE_URL="http://127.0.0.1:$PORT"
GATEWAY_SESSION="cowd-v0952-gateway-$$"
TUI_SESSION="cowd-v0952-tui-$$"
TMP_DIR="$(mktemp -d /tmp/cowd-v0952-tui-daemon.XXXXXX)"
...
SCENARIO_API_KEY="${ANTHROPIC_API_KEY:-test-dummy-key-for-v0952-scenario}"
...
on_error() {
  local status=$?
  echo "v0.9.52 TUI daemon attach scenario failed with status $status" >&2
  print_logs
  exit "$status"
}
...
for cmd in tmux curl rg ss sqlite3; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "$cmd is required for v0.9.52 TUI daemon attach scenario" >&2
    exit 1
  fi
done
...
curl -fsS -X POST "$BASE_URL/api/connectors/services/mock.docs/execute" \
  ...
  --data "{\"actor_principal\":\"user:v0952\",\"tool_id\":\"service.mock.docs.read\",\"resource_id\":\"v0952-tui-doc\",\"title\":\"v0952 TUI Attach Doc\",\"mode\":\"commit\",\"idempotency_key\":\"v0952-$$\"}" \
  >/dev/null
...
sqlite3 "$WORKDIR/.cowd/resource-directory.sqlite" "SELECT reference FROM connector_resources;" \
  | rg -q "service://mock.docs/document/v0952-tui-doc"

tmux kill-session -t "$TUI_SESSION" >/dev/null 2>&1 || true
tmux kill-session -t "$GATEWAY_SESSION" >/dev/null 2>&1 || true
echo "v0.9.52 TUI daemon attach scenario passed"
