...
TMUX_PATTERN='cowd-|webui-|phase5|message100k|prepare|audit|workspace|profile|v0815|v0816'
...
check_no_test_ports() {
  printf '\n==> checking v0.8.16 test ports\n'
  if ss -ltnp | rg "$PORT_PATTERN"; then
    echo "v0.8.16 eval found leftover test listeners" >&2
    return 1
  fi
}

check_no_test_tmux() {
  printf '\n==> checking v0.8.16 tmux sessions\n'
  if ! command -v tmux >/dev/null 2>&1; then
    echo "tmux not installed; skipping tmux check"
    return 0
  fi
  if ! tmux ls >/tmp/cowd-v0816-tmux.txt 2>/dev/null; then
    return 0
  fi
  if rg "$TMUX_PATTERN" /tmp/cowd-v0816-tmux.txt; then
    echo "v0.8.16 eval found leftover test tmux sessions" >&2
    return 1
  fi
}

maybe_live_scenarios() {
  if [ "${COWD_V0816_LIVE:-0}" != "1" ]; then
    printf '\n==> skipping optional live tmux/browser scenarios (set COWD_V0816_LIVE=1)\n'
    return 0
  fi
...
main() {
  ...
  printf '\nv0.8.16 runtime cortex evaluation passed\n'
}
