...
- `scripts/parity/run-mock-parity-harness.sh` — convenience wrapper
...
```bash
cd rust/
./scripts/parity/run-mock-parity-harness.sh
```
...
```bash
cd rust/
python3 scripts/parity/run-mock-parity-diff.py
```

Scenario-to-PARITY mappings live in `scripts/parity/mock-parity-scenarios.json`.
